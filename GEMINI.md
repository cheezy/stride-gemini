...
- **task-reviewer** — Review changes against acceptance criteria before completion (medium+ complexity or 2+ key_files). Emits a structured `reviewer_result` block (`schema_version` 1.6: `status`, `issue_counts`, `issues[]`, `acceptance_criteria[]`, `project_checks[]` from a project-root `CODE-REVIEW.md` with per-entry `status` enum `met`/`not_met`/`not_applicable` and full-checklist emission, and per-section `testing_strategy`/`patterns`/`pitfalls`/`security_considerations` verdicts, plus an OPTIONAL `behaviour_test_matrix` verdict with a per-row breakdown, emitted only when the task supplies a matrix). Persist it verbatim per `stride-workflow` Step 5; schema owned by `agents/task-reviewer.md`.
...
When the companion `stride-gemini-exploratory-testing` extension is installed, `stride-workflow` runs an **optional, gated** manual-testing step (**Step 5.5**, between Code Review and Execute Hooks; documented as **Phase 3.5** in `stride-subagent-workflow`). It triggers only when the task's `testing_strategy.manual_tests` is non-empty AND that extension is available — detected **availability-only** by its sanctioned command/agent/skill surface (never by reading, sourcing, or eval'ing extension files). When available it dispatches the extension's **`explorer` custom agent** — the only surface that completes without a human, never `/explore`, `/pair`, `/recon`, `/nightmare-headline` or the routing skill — mapping each manual test to a charter and passing one environment-context block carrying an explicit session budget and the user's authorized, non-production affirmative (no affirmative, no dispatch). Budget exhaustion is a normal outcome that never fails completion. It records the findings in existing completion fields (`completion_notes`, and the `reviewer_result.testing_strategy` note when a reviewer ran) — no new completion field. When the extension is absent or the task has no manual tests, the workflow falls back with **no failure**. Dispatched testing stays within the exploratory-testing safety boundary: authorized, non-production targets only, no destructive or production-mutating actions.
