...
**Stated here, owned there — entries `decision-matrix-authority` and `row-precedence` of `stride/docs/port-canon.md`.** The sentence beneath the table answers to the first; the ordering, its argument and the worked collisions answer to the second. Move the substance of either and the bump is owed twice before anything ships — once in that entry, once on the anchor that opens it.
...
- **`review_round` — asserted by you, and NOT part of the task-supplied bullet above.** It is a lifecycle fact only the orchestrator holds, so it travels alongside that list rather than inside it, and the reviewer's "a field is absent only when the task left it empty" rule does not reach it. Send `{ "round": <n>, "fixes": [ { "ref": "<severity> <category> <file>:<line>", "change": "<one line>" } ] }` on every round after the first; **absent means round 1**. Each `fixes[]` entry carries a reference and a single line of description — **never the previous round's block, its prose, or diff text** — and inherits this workflow's redaction rules whole: a `ref` that would name a secret, or say where one lives, is redacted exactly as any other session text is. What the reviewer does with it is owned by `agents/task-reviewer.md`.
...
**Two review rounds is the ceiling, and round two verifies rather than reviews again.** A reviewer asked to look will always find something, so a loop with no ceiling never converges — it only gets more expensive. What counts as a **round** has to be said in terms this port actually carries, because there is no merged-result file here to point at: a round is **an invocation of the `task-reviewer` custom agent whose response came back carrying a fenced ```json block that parsed**. An invocation that crashed, returned nothing, or came back with a block that would not parse is re-invoked and costs no round — that is the same line the **Fallback when JSON parsing fails** section below already draws between a review you can use and one you cannot, reused here rather than invented. **Re-invoke once, not indefinitely.** If the second invocation also comes back unusable, stop re-invoking and take that fallback path, which ships a degraded-but-valid completion. Costing no round is not a licence to loop: the fallback is what bounds this, and it is the disposition that section actually carries — the line it draws is shared, the remedy is not. Nothing is written to disk per invocation. A re-invocation costs no ROUND — that follows from the parse-based definition two sentences up, not from the disk fact — but be careful not to read it as costing nothing. It spent its tokens, and **it counts for `dispatch_count`**: tick your running tally for it exactly as for an invocation that came back clean. Rounds and dispatches are different quantities, and this is the one moment where they visibly diverge.

**Round two is narrowed in what it hunts for, never in what it is shown.** It receives the **full** task diff and every review field the task supplies, exactly as round one did, and it still emits the whole `acceptance_criteria` array, all four section verdicts and the complete `project_checks` list. Only its mission narrows: verify the fixes named in `review_round.fixes[]`, and re-check what those fixes could plausibly have broken. Handing round two the fix delta *instead of* the diff is what would turn its 1:1 criteria array into a fiction — the same corruption the paragraph above exists to prevent, reached from the other direction.

**Once two rounds are done, the `important` and `minor` findings still standing are RECORDED rather than fixed.** Write each one as **severity, category and a repository-relative `file:line` — nothing further** — into `completion_notes` **and** into one line of `completion_summary`, and include any round-one finding round two never mentioned again. The block, its prose and diff text stay out of both, redacted on the same terms as anything else this session writes; those two fields are sinks this workflow's redaction rules already cover, which is precisely why recording goes there and nowhere new.

**Recording is closed to a `category: "security"` issue at every severity, without exception.** `important` is the severity the task-reviewer defaults to for a security finding, so a rule written for wrapping, counts and phrasing would quietly become the route by which a real weakness ships unfixed. Fix a security finding, and run a further round scoped to that fix to verify it — that round is permitted at the ceiling on the same terms the `critical` exemption grants below, because verifying a security fix is correctness, not process. If you cannot fix it, **DO NOT submit**: stop short of completing, leave the task claimed, and put the finding in front of the human in this session. Never record it.

**A `critical` finding is exempt from the cap and blocks in whatever round it surfaces.** The ceiling is on rounds, not on correctness. Fix it and run a further round scoped to that finding, as many times as that takes. If you cannot fix it, **DO NOT submit**: stop short of completing, leave the task claimed, and put the finding in front of the human in this session.

**If a third reviewing round has already run, there is still an exit — take it rather than stalling.** A round count only ever goes up, so re-invoking the reviewer cannot walk it back and the completion gate will not go green on a later evaluation; waiting is not a strategy and neither is another round. Record the residual `important` and `minor` findings exactly as at the ceiling and submit **round two's** result, which is the last one you were entitled to. If you no longer hold it, **DO NOT submit**: stop short of completing, leave the task claimed, and tell the human in this session that the review loop over-ran its cap and by how much. Saying so is the point — an over-run that is reported is a process problem, while one that is quietly absorbed is the same over-run with the evidence removed.

**Stated, not pinned — worth saying plainly rather than leaving to be inferred.** Nothing in this port executes this **round cap**. There is no round counter on disk, no boolean that can come back false, and the pre-submission gate in `stride-completing-tasks` is prose you apply to yourself rather than a script that runs. The count is whatever this session observed, which makes it **self-certified**, on the same footing as everything else here you assert rather than compute. Two things follow and both bind: never describe a round as having been cleared mechanically, and where a resumed session cannot reconstruct how many rounds parsed, count the next one as round **two** for the ceiling — but send it **no** `review_round` at all, so it reviews the full diff unnarrowed instead of verifying a `fixes[]` list nobody gave it. Both halves are load-bearing, and the second is the one that is easy to drop: counting it as round two resolves the doubt toward the ceiling, while withholding the narrowing stops that same doubt resolving into no substantive review at all. A round told to verify an empty fix list is told to look nowhere, and spending the ceiling on one would be the expensive way to review nothing.
...
- **Keep a running tally of reviewer dispatches, and tick it here rather than reconstructing it later.** Every time you invoke the `task-reviewer` custom agent, add one — including an invocation that crashed, returned nothing, or came back unparsable, because it spent its tokens even though it bought no round. That tally is `dispatch_count` on the `reviewer` `workflow_steps` entry (see § Workflow Telemetry). **It is asked for here, at the dispatch, because this port keeps no per-invocation artifact to recover it from afterwards** — unlike `duration_ms`, which you can still read off a clock at completion time, a count of past events is only knowable if somebody was counting. **If you reach completion without such a tally — a resumed or compacted session, or a handoff — OMIT `dispatch_count` rather than estimating it.** An honest absence is a case the limits know how to read; a plausible guess is the failure those limits exist to prevent.
- **Save the reviewer's full response (prose + JSON block)** -- you'll include it verbatim as `review_report` in Step 7

**Stated here, owned there — entry `review-round-cap` of `stride/docs/port-canon.md`.** This section is a place the rule is spoken; the canon is where it lives. Move its substance and the bump is owed twice before anything ships — once in that entry, once on the anchor overhead.
...
3. **Record the deep dispatch's time under the existing `reviewer` `workflow_steps` entry — do NOT add a new step name.** **Fold the wall-clock only, never the count** — `dispatch_count` counts `task-reviewer` dispatches and the security specialist is a different subagent, so it does not tick. Fold its wall-clock into the reviewer step's `duration_ms`; the deep review is part of the review phase, not a separate telemetry step.
...
6. **Telemetry:** fold this session's wall-clock into the existing **`reviewer`** `workflow_steps` entry, exactly as the deep security-considerations review and Step 5.6 do. **Fold the wall-clock only, never the count** — `dispatch_count` on that entry counts `task-reviewer` dispatches, and this is a different subagent, so it does not tick. **Do not add a seventh step name** — the vocabulary is fixed at six, and `stride-completing-tasks` separately forbids recording this dispatch as a seventh. Note the wall-clock is **your own measurement of the dispatch**, never a field read out of the session sheet: today's contract states outright that it carries no `duration` and no `tbs`, so there is nothing there to read. When no reviewer ran, that entry is the skip form and carries no duration; record the dispatch in `completion_notes` instead rather than inventing a duration for a step that did not run. **That case is not an edge case here** — this step's gate has no review precondition, so the small 0-1 `key_files` path reaches it routinely with no *dispatched* reviewer entry to fold into. The entry itself is still submitted, as `dispatched: false` with a reason: all six names are always present, and dropping one would be the incomplete-telemetry record this rule exists to prevent.
...
**Telemetry:** fold this dispatch's wall-clock — **and only the wall-clock, never the count, since `dispatch_count` counts `task-reviewer` dispatches and this is a different surface** — into the existing **`reviewer`** `workflow_steps` entry, exactly as the deep security-considerations review does. **Never a seventh step name** — the vocabulary is fixed at six. When no reviewer ran, that entry is the skip form and carries no duration, so record the dispatch in `completion_notes` rather than inventing one.
...
| `dispatch_count` | integer | Optional, and only alongside `dispatched=true` | How many times this step's subagent was **dispatched**. It counts dispatches, **not rounds** — a dispatch that crashed and was re-dispatched still spent its tokens, so it counts, while the round counter deliberately excludes it. Adds no seventh step name: a new key is not a new name. Leaving it off is never an error, so a version of this extension that predates it completes exactly as before — but state a `1` you actually know, because an omission you chose looks identical to one a plugin could not avoid. **If a resumed session cannot reconstruct the tally, omit the key rather than estimate** — the round count above at least has parsed blocks in context to reason from, and this has less. **Read the limits below before drawing any conclusion from it** |
...
**Six limits, and they are the more important half of this feature.** A figure trusted past its accuracy is worse than no figure, so these are stated here rather than left for a reader to discover. The pair exists to stop the review phase's cost being invisible, and it does that. It does **not** rank two tasks by cost, and that distinction is the whole point of this section.

**(1) Wall-clock is not token cost, and the two disagree about which task was dearer.** Measured across eight real dispatches, tokens per second of wall-clock varied about **2.1×** by dispatch kind — reviewer rounds around 300-375 tok/s, specialist security dispatches around 455-625. So the figure tracks a task's review *mix*, not its size. On the two real records available when this was written the pair called one task 20.3% more expensive when its token cost was in fact 1.4% **cheaper** — the order inverted and the recorded gap ran about fifteen times the real one. **Use the pair to see that a review phase was long; never to conclude it was the costlier of two.**

**(2) The two keys count different populations, so never divide one by the other.** `duration_ms` on this entry is an aggregate — the deep security-considerations review, the Step 5.5 session and Step 5.6 hardening all fold their wall-clock into it, exactly as Step 5.5 above instructs. `dispatch_count` counts only this step's own subagent, the reviewer, and is therefore usually the smaller number. On those same two records the duration covered four dispatches while the count reported two, and dividing overstated the mean reviewer round by **40% and 52%**. The bias is not constant, so it cannot be corrected away, and the composition is written down nowhere. **There is no per-round figure in this record. Do not compute one.**

**(3) A review-skipped task can have a real review-phase cost and record none of it.** The deep security review and the Step 5.5 session each gate on their own conditions with **no reviewer precondition**, so both can run on a task whose decision-matrix row skipped review. There the reviewer entry is the skip form — `dispatched: false`, no duration, no count — and a task that ran a security specialist and a full exploratory session is byte-identical, in `workflow_steps`, to one whose review genuinely cost nothing. Those dispatches go into `completion_notes` prose, which nothing aggregates. **An absent cost figure is not evidence of absent cost.**

**(4) An omitted count is ambiguous, and a reader must not impute one.** It may mean a version predating the key, an oversight, or — the case this port makes likeliest, and the one the taxonomy inherited from the reference did not have to carry — **a session that honestly could not reconstruct the tally and correctly declined to invent one.** That third meaning is the disciplined outcome, so do not read an absence as negligence. It should not mean a `1` the author knew and declined to write — the rule above says state it — but nothing in the record lets a reader tell those apart. Imputing `1` rather than a higher value moved a sample mean by over 100%. **When aggregating, report the covered subset and its size rather than filling in absences**, and note the ambiguity is widest during rollout, which is precisely when aggregating is most tempting.

**(5) It cannot separate a crashed re-dispatch from a genuine extra round, and that is deliberate.** A `2` may be one round plus one crash, or two rounds — **or, on the fallback path below, two unusable invocations and no round at all**, since re-invoking once before falling back leaves two dispatches that each spent tokens while nothing ever parsed. Do not read a count as evidence that any round completed. The count exists to capture cost, and a crashed dispatch really did spend its tokens — but the consequence is worth stating plainly: a fully compliant `3`, two rounds plus a crash, reads like a breach of the two-round cap to anyone who knows that cap. It is not one. **The round number is not in the completion payload at all**, so the distinction cannot be recovered from the record afterwards. That is a reason to say in `completion_notes` what a count above two meant — **in your own words, as dispatch bookkeeping rather than a description of what any dispatch found**, redacted on exactly the terms that already govern anything reaching that field, and never quoting reviewer prose or session output — and never to read a cap breach out of `dispatch_count` alone.

**(6) Nothing validates the value on the way in, so the first consumer must guard it.** The shared backend's `valid_step?/1` checks `name`, `dispatched`, `duration_ms` and `reason`, and separately the `reason_code` enum — but **not** this key. Unlike `reason_code`, which is rejected when unrecognised, `dispatch_count` is accepted whatever its type, persisted into the array, and returned verbatim by the API. Nothing reads it today, so the present cost is a corrupt record and nothing worse. **Whoever reads it first — a renderer, a dashboard, an aggregation — guards for a non-integer itself, or the validator gains the optional-but-validated shape `reason_code` already has.** Be concrete about where that lands, because this file is the *writer's* contract and the party who must discharge the obligation works somewhere else entirely: the check would go in `Kanban.Tasks.WorkflowSteps.valid_step?/1`, and the note recording the decision belongs in that module's `@moduledoc`, beside the existing passage explaining why `name` is deliberately left unconstrained — that passage is the precedent, and its absence for this key is the gap. **An obligation stated only where the writer reads it has not actually been routed to its assignee**, so a reader of this line who is in a position to discharge it should put the note there rather than leave it here. It is assigned here so the next person to add a key does not inherit the absence as though it were a design decision.

**Why not simply record tokens.** Tokens are the only figure observed to rank correctly, so it is the obvious answer to limit (1) — and the usual justification for refusing is wrong: some runtimes do measure a per-dispatch token cost, so recording it would not be inventing one. The real objection is **portability**, whether every runtime this schema serves can produce one, and that is unsettled. Until it is, this pair is what there is, read within the limits above.

**Stated here, owned there — entry `dispatch-count-telemetry` of `stride/docs/port-canon.md`.** The limits above are a place the rule is spoken; the canon is where it lives. Move their substance and the bump is owed twice before anything ships — once in that entry, once on the anchor overhead.
...
**Stated here, owned there — entry `reason-code-vocabulary` of `stride/docs/port-canon.md`.** The table above and the sentences around it are a place the rule is spoken; the canon is where it lives. Move their substance and the bump is owed twice before anything ships — once in that entry, once on the anchor overhead.
...
- Add `dispatch_count` to a `dispatched: true` entry whenever you dispatched its subagent more than once, and state a `1` you know rather than omitting it. **Count dispatches, including one re-dispatched after a crash; never fill it from a round counter, which deliberately excludes those.** Omitting it stays valid. **Read the six limits above before drawing any conclusion from the figure** — in particular, `duration_ms` divided by `dispatch_count` is not a per-round figure, and wall-clock is not token cost.
