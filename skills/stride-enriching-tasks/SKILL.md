...
- `pitfalls`: Read GEMINI.md/AGENTS.md → found dark mode verification requirement; read `view_component.ex` → found no timezone handling
...
2. **Search for existing modules:**
   ...
   grep_search pattern="pagination|paginate" path="lib/"
   grep_search pattern="def.*task.*list|def.*list.*task" path="lib/"
   ```
3. **Search for related LiveViews/controllers** if the task is UI-related:
   ```bash
   grep_search pattern="task" path="lib/kanban_web/live/" output_mode="files_with_matches"
   ```
4. **Search for context modules** if the task involves data/business logic:
   ```bash
   grep_search pattern="def.*task" path="lib/kanban/" glob="*.ex" output_mode="files_with_matches"
   ```
...
grep_search pattern="error message text" path="lib/"
...
2. **Find the closest analog** — a feature similar to what's being built:
   ...
   grep_search pattern="paginate|page_size|offset" path="lib/"
   ```
...
No similar feature exists?
  → Note the general project conventions (from GEMINI.md/AGENTS.md patterns)
```
...
1. **Map key_files to test files:**
   ...
   read_file file_path="test/kanban/tasks_test.exs"
   ```
...
5. **Check GEMINI.md/AGENTS.md** for project-specific pitfalls (dark mode, translations, etc.)
...
1. **Broaden the search** — use fewer keywords or synonyms
   ...
   grep_search pattern="page|limit|offset" path="lib/"
   ```
...
1. **Search all candidate areas** and compare relevance
   ```bash
   grep_search pattern="task" path="lib/kanban/" output_mode="files_with_matches"
   grep_search pattern="task" path="lib/kanban_web/" output_mode="files_with_matches"
   ```
...
1. **Search `mix.exs` for related dependencies:**
   ```bash
   grep_search pattern="dep_name" path="mix.exs"
   ```
...
2. In Phase 2, use the title keywords more aggressively:
   ```bash
   grep_search pattern="search" path="lib/" output_mode="files_with_matches"
   grep_search pattern="search" path="test/" output_mode="files_with_matches"
   ```
...
- Step 1: Search for error messages, stack traces, or the buggy behavior in code
  ```bash
  grep_search pattern="error message from bug report" path="lib/"
