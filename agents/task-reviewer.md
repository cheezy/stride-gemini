...
**Narrowing changes what you look for, never what you emit.** The `acceptance_criteria` array is still one entry per task criterion line, verbatim and in the task's order; all four section verdicts, `project_checks`, `issue_counts` and `issues` are still emitted in full; and `schema_version` is `"1.7"` — round scoping itself adds no field to the block; the bump came from the `cosmetic` key added below. You are handed the **full** task diff on every round, round two included.
...
8. **Return Structured Review**:
   ...
   - The JSON object has these top-level fields (all required unless explicitly marked OPTIONAL, snake_case throughout):
     - `schema_version`: string. Always `"1.7"` for this prompt version. (Bumped from `"1.6"` by the addition of the `cosmetic` key to `issues[]` entries; the two changes before it held at `"1.6"` precisely because they added no field. Nothing gates on the value — the server validates it as a semver-shaped string only.)
     ...
     **`cosmetic` — presentational only, and it changes disposition, never the finding.** Set it `true` only when **both** claims hold: the *finding's* claim is correct, **and** the *artifact* it points at asserts nothing that is itself false **and misleads no reader**. The second gate has two halves and the misleading half is the one that gets dropped: an artifact can mislead without asserting anything false — a correct sentence under a heading that scopes it wrongly, an accurate example that misrepresents the rule it illustrates, a re-ordering that leaves *the above* or *the following* binding to something new. Those are substantive, and the gate says so here rather than leaving it to be recovered from the never-a-downgrade paragraph below. The false half is easy to miss for a different reason: a finding about a false statement is itself perfectly correct — a document saying "prints six values" where seven print, a comment placing an assert below a write that sits above it, a heading reading "Two limits" over three. In each the finding is right, which clears the first gate; what makes them **substantive** is that the artifact states something false. **A false statement of fact is never cosmetic**, however small, and however much its subject looks like a count or a wording. With both gates clear, the subject must then be purely presentational: line wrapping, column width, word or line counts, phrasing preference, ordering of otherwise-equivalent prose. **Read that list as examples of a test, never as a list to match against** — the same edit can be either. Re-wrapping a paragraph is cosmetic; re-wrapping inside anything another process extracts — a shell command, a heredoc, a code fence something greps or extracts, **or a prose sentence a test greps as a needle** — changes what runs and is **substantive**. That last one is not hypothetical here: this port's own hook suite greps sentences out of these contract files and compares them across files byte for byte, so the category is *what extracts it*, never *whether it looks like code*, as is a re-ordering in a document whose structure is load-bearing. Apply the test, not the examples. Set it `false`, or omit it, on everything else.

     **What it does NOT do — four of them, and each is a way the flag could be turned into a suppression.** It does **not** change `severity`: a cosmetic finding carries the same honest severity it would have carried without the flag. The flag is refused above `minor`, so a presentational finding that honestly rates higher is reported **without** it — never re-rated downward to keep it. That is stated as a stipulation rather than a consequence on purpose: presentational does not *imply* `minor`, and a reason of that shape would hand a reviewer a principled-sounding argument for the one downgrade this paragraph exists to forbid. It does **not** change `category`: a cosmetic `code_quality` finding stays `code_quality`. It does **not** change `status`: `status` is driven by `critical`/`important` issues and by `not_met` criteria and checks, none of which a `minor` finding touches, so the status rule above is unaffected and needs no exclusion added to it. And it does **not** remove the finding from `issues[]`, from `issue_counts`, from your prose summary, or from anything downstream — a cosmetic finding is reported exactly like any other and reaches `completion_notes` on the same terms. **The single thing it changes is the orchestrator's re-review disposition**: a round whose findings are *all* cosmetic buys no further review round. That is why it is a disposition rather than a carve-out, an exclusion, or a suppression — it removes no finding from any record and excuses no verdict.

     **It is never a way to downgrade a real finding.** `cosmetic: true` on a finding whose claim is *substantive* — anything where the code or the contract would behave differently, or a reader would be misled — is a **reviewer defect**, not a judgement call. The test is not "is this small" but "is the only thing wrong here how it looks". A one-character typo that inverts a meaning is substantive; a line four columns too wide is cosmetic. **Cosmetic is orthogonal to severity, not a fourth level of it**: it only ever sits on a `minor`, and a `minor` can perfectly well be substantive — a real but low-impact defect — and those still buy a round.

     **Three conditions are refused.** `cosmetic: true` is invalid when `severity` is anything other than `"minor"` — which covers **`critical` and `important` alike** — when `category` is `"security"`, or when `cosmetic` is not a real boolean (no coercion: `1`, `"yes"` and `"true"` are all refused). Put positively: never `cosmetic` on a `critical`, an `important`, or a `category: "security"` finding. **Refused by you, not by a script.** This port runs no `cosmetic_shape_ok`: the reviewer returns its block inline in its response and no completion-time gate here parses it for a shape pin, so these three are prose you apply as you emit the block — self-certified, on exactly the footing the review-round cap already records for itself in `skills/stride-workflow/SKILL.md`. Never describe one as having been cleared mechanically. Two limits follow, and both are worth stating rather than leaving to be inferred. First, nothing here checks the flag's *truth*, only a reader can, so the cheapest abuse is not relabelling a `critical` or a `security` finding but relabelling an ordinary substantive `minor`. Second, excluding `category: "security"` excludes a **category, not a subject matter** — a security-relevant observation filed under `code_quality` is not reached by that exclusion. That boundary is reused deliberately rather than a new one invented, because `category` is the boundary the rest of this file already keys security handling on; a finding you would be uncomfortable seeing skipped belongs in `security`, and marking it cosmetic anywhere else is the same reviewer defect the paragraph above names.

     **Stated here, owned there — entry `cosmetic-finding-class` of `stride/docs/port-canon.md`.** These paragraphs are a place the rule is spoken; the canon is where it lives. Move their substance and the bump is owed twice before anything ships — once in that entry, once on the anchor overhead.
     ...
     **Stated here, owned there — entry `verdict-note` of `stride/docs/port-canon.md`.** The two rules above are a place the rule is spoken; the canon is where it lives. Move their substance and the bump is owed twice before anything ships — once in that entry, once on the anchor overhead.
